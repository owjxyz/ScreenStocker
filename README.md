...
관리 앱에서 Twelve Data API 키를 저장하면 화면보호기가 실제 Twelve Data `/quote` 응답을 사용합니다. 관심 종목 입력창에 회사명이나 티커를 넣고 `Search`를 누르면 Twelve Data `/symbol_search` 결과에서 종목을 추가할 수 있습니다. API 키가 비어 있거나 네트워크 응답이 실패하면 개발용 데모 시세로 표시됩니다.

화면보호기 설정 창에서는 문자열을 직접 입력하지 않고, 관리 앱에 등록된 첫 종목 또는 개별 종목을 드롭다운 메뉴에서 선택합니다. 화면보호기는 한 번에 한 종목의 현재가와 Twelve Data `/time_series` 기반 라인 차트를 화면 전체에 표시합니다.
